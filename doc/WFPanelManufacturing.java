 ...
 * Workflows
 */
public class WFPanelManufacturing extends ADForm {
	private static final long serialVersionUID = 1L;
	private static CLogger log = CLogger.getCLogger(WFPanelManufacturing.class);
	...
	protected void initForm() {
		...
		String sql = "SELECT AD_Workflow_ID, Name FROM AD_Workflow " + "WHERE AD_Client_ID=? AND IsActive='Y' "
				+ "AND (WorkflowType='M' OR Value LIKE '%Manufacturing%') " + "ORDER BY Name";
		...
		// -- WEST: Standard Op Library --
		org.zkoss.zul.West west = new org.zkoss.zul.West();
		west.setWidth("200px");
		west.setTitle("Standard Ops");
		...
		String[] stdOps = new String[] { "Cut", "Sew", "Iron", "Pack", "Inspect", "Ship" };
		for (String op : stdOps) {
	...
	public void onEvent(Event event) throws Exception {
		...
		if (event.getTarget().getId().equals(ConfirmPanel.A_CANCEL))
			...
			if (dragged instanceof ListItem) {
				// Handle Standard Op Drop
				String opName = (String) ((ListItem) dragged).getValue();

				Integer targetNodeID = (Integer) target.getAttribute("AD_WF_Node_ID");
				if (targetNodeID != null && opName != null) {
					// Drop on Existing Node -> Insert Logic
					insertStandardOpOnNode(opName, targetNodeID);
				} else {
					...
					if (opName != null && m_wf != null && xPosition != null && yPosition != null) {
						int AD_Client_ID = Env.getAD_Client_ID(Env.getCtx());
						MWFNode node = new MWFNode(m_wf, opName, opName);
	...
	private void insertStandardOpOnNode(String opName, int targetNodeID) {
		...
		// 1. Create New Node
		MWFNode newNode = new MWFNode(m_wf, opName, opName);
	...
	protected void showNodeMenu(Component target) {
		...
		if (AD_WF_Node_ID != null) {
			...
			if (widget != null) {
				...
				// -- Assign Resources (Garment Feature) --
	...
	private void executePopupAction(Menuitem item) {
		if (item.getAttribute("MWFNodeNext") != null) {
			...
			if (AD_WF_NodeTo_ID == WFPopupItem.WFPOPUPITEM_ZOOM) {
				AEnv.zoom(128, node.getAD_WF_Node_ID()); // 128 = AD_WF_Node
